...
from dataset3 import build_loader
...
def get_net(cfg):
    ...
    checkpoint = torch.load(cfg.initial_checkpoint)
...
class Fitter:
    def __init__(self, config):
        ...
        self.model = get_net(config).to(device)
        self.device = device
    ...
    def validation(self, val_loader):
        ...
        t = time.time()
        bar = tq(val_loader, postfix={"summary_loss": 0.0})
        for step, (images, targets, image_ids) in enumerate(bar):
            if self.config.verbose:
            ...
            with torch.no_grad():
                ...
                bar.set_postfix(ordered_dict={"val_loss": loss.detach().item()})
    ...
    def train_one_epoch(self, train_loader):
        ...
        summary_loss = AverageMeter()
        t = time.time()
        bar = tq(train_loader, postfix={"summary_loss": 0.0})
        for step, (images, targets, image_ids) in enumerate(bar):
            if self.config.verbose:
                if step % self.config.verbose_step == 0:
                    print(
            ...
            summary_loss.update(loss.detach().item(), batch_size)
            bar.set_postfix(ordered_dict={"train_loss": loss.detach().item()})
